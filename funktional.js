...
(function() {
    ...
    const suchKonfigurationen = [
        ...
        { begriffe: ['dach himmel anth', 'himmel anth', 'Dachhimmel schwarz'], anzeige: 'Dachhimmel Anthrazit / Schwarz', aktiv: true },
    ...
    function getGesamtText() {
        ...
        if (beschreibungsBereich) {
            const txt = beschreibungsBereich.textContent.trim();
            if (txt) textParts.push(txt);
        }
    ...
    function technischeDatenHinzufuegen(parentElement) {
        ...
        technischeDaten.forEach(d => {
            ...
            dataElement.textContent = d.title + ": " + d.value;
            dataElement.style.color = "white";
            techContainer.appendChild(dataElement);
        });
